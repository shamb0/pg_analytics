...
#[rstest]
async fn test_obj_cache_perf_on_disk_on(
    #[future] s3: S3,
    mut conn: PgConnection,
    parquet_path: PathBuf,
) -> Result<()> {
    ...
    // Set up the necessary tables in the PostgreSQL database using the data from S3.
    AutoSalesTestRunner::setup_tables(&mut conn, &s3, s3_bucket, true).await?;
...
#[rstest]
async fn test_obj_cache_perf_on_disk_off(
    #[future] s3: S3,
    mut conn: PgConnection,
    parquet_path: PathBuf,
) -> Result<()> {
    ...
    // Set up the necessary tables in the PostgreSQL database using the data from S3.
    AutoSalesTestRunner::setup_tables(&mut conn, &s3, s3_bucket, false).await?;
